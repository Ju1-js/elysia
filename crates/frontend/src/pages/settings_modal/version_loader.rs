...
pub async fn load_wine_versions(
    ...
    match service.get_wine_status(settings, &wine_runner).await {
        Some((_, current, versions)) => {
...
pub async fn load_proton_versions(
    ...
    match service.get_proton_status(settings, &proton_runner).await {
        Some((_, current, versions)) => {
            debug!("Current installed: {:?}", current);
            debug!("Available: {} versions", versions.len());
...
pub async fn load_dxvk_versions(
    ...
    match service.get_dxvk_status(settings, &wine_runner).await {
        Some((_, current, versions)) => {
            debug!("Current installed: {:?}", current);
            debug!("Available: {} versions", versions.len());
